<?php include_once('w34CombinedData.php');include_once('common.php');error_reporting(0);
if($theme==="dark"){$stroke="#ffd348";}
else if($theme==="light"){$stroke="#faa716";}
# heavily modified version of Wim weather display version cleaner output
$DateTimeZone = 'Europe/'.$TZ; 
...
$nextfirstlight = $result['civil_twilight_begin']; 
$now = time();if ($now > $nextfirstlight) {
$nextfirstlight = date('H:i',$result2['civil_twilight_begin']); 
} else { $nextfirstlight = date('H:i',$nextfirstlight); } 
//weather34 sunset last light
$nextlastlight = $result['civil_twilight_end'];
if ($now > $nextlastlight) { 
$nextlastlight = date('H:i',$result2['civil_twilight_end']);
} else { $nextlastlight = date('H:i',$nextlastlight); }  
...
$hrs = gmdate ('G',$time); 
$min = gmdate ('i',$time); 
$txt = '<value>&nbsp;Till Sunrise';
} elseif ($now < $firstset) { 
$time = $firstset - $now; 
$hrs = gmdate ('G',$time); 
$min = gmdate ('i',$time); 
$txt = ' &nbsp;<value>Till Sunset';
} else { $time = $secondrise - $now; 
$hrs = gmdate ('G',$time); 
$min = gmdate ('i',$time); 
$txt ='&nbsp;<value>Till Sunrise'; } echo "</value>";
...
$h = $hour + $min / 60;
$gmst = (6.697375 + 0.0657098242 * $time + $h);
$gmst = fmod($gmst,24);
...
$long = $lon;
$timezone = $TZ;
$_SunPos = new sunPos($lati, $long, $timezone);
$azimuth = $_SunPos->azimuth;
$elev = $_SunPos->elevation;
$light = $weather["daylight"]; 
$daylight = ltrim($light, '0'); 
$dark = 24 - str_replace(':', '.', $daylight);
$lighthours = substr($daylight, 0, 2); 
...
$thehour = date('H'); // minus 1 hour for CEST
$theminute =date('i');
?>
...
<?php if ($elev >= 0) { 
$elev1 = $_SunPos->elevation."&deg;<div class=sunaboveweather34>&nbsp;</div>"; 
} else if ($elev < 0) { 
